#!/usr/bin/env bash
# vault-lint-cron.sh — runs daily 03:00 local via launchd (com.brain.vault-lint).
# Invokes /vault-lint via `claude -p`.
#
# Gates:
#   • Same-day dedup (.last-run marker, written only on success)
...
VAULT="${VAULT_LINT_VAULT:-$HOME/work/brain}"
STATE_DIR="${VAULT_LINT_STATE_DIR:-$HOME/.local/state/vault-lint}"
LOG_FILE="$STATE_DIR/cron.log"
LAST_RUN="$STATE_DIR/.last-run"
ENV_FILE="${VAULT_LINT_ENV_FILE:-$HOME/.config/brain/env}"
USAGE_FILE="${VAULT_LINT_USAGE_FILE:-$HOME/.cache/ccstatusline/usage.json}"
WEEKLY_THRESHOLD="${VAULT_LINT_THRESHOLD:-80}"
MODEL="${VAULT_LINT_MODEL:-sonnet}"
...
log "=== vault-lint tick (dry_run=$DRY_RUN model=$MODEL) ==="
...
# ---- Same-day dedup (defensive against double-fire; StartAtLoad is off so
# normally only one fire per day, but a manual re-run shouldn't double up).
TODAY=$(date "+%Y-%m-%d")
...
if (( DRY_RUN == 1 )); then
  log "dry-run: skipping claude invocation; would run: claude --dangerously-skip-permissions --model $MODEL -p /vault-lint"
  log "=== vault-lint done (dry-run) ==="
  exit 0
fi

# ---- Invoke /vault-lint
log "invoking: claude --dangerously-skip-permissions --model $MODEL -p /vault-lint"
if cd "$VAULT" && claude --dangerously-skip-permissions --model "$MODEL" -p "/vault-lint" >> "$LOG_FILE" 2>&1; then
  log "vault-lint completed successfully"
  echo "$TODAY" > "$LAST_RUN"
  log "=== vault-lint done ==="
  exit 0
else
  RC=$?
  log "vault-lint failed (exit=$RC)"
  send_telegram "*Vault Lint Failed*: \`/vault-lint\` cron exited $RC on $TODAY. Check \`$LOG_FILE\`."
  log "=== vault-lint done (failed) ==="
  exit "$RC"
fi
